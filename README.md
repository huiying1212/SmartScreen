...
默认查询范围：过去 1 天 + 未来 1 天，最多 20 条事件。
...
- 根据 LLM 评分（0-100 → 0.0-1.0）连续插值渲染面部表情（无离散阶段）
...
| `ContextSnapshotCollector` | 统一上下文快照构建，供 LLM 评分和气泡文案使用 |
| `LocationContextInferrer` | 基于活动类型 + 定位精度 + 时段推断位置场景 |
| `DataPersistenceManager` | 数据持久化（加密/压缩/明文 JSON） |
| `DataSanitizer` | PII 脱敏处理 |
...
| 类 | 职责 |
|---|---|
| `DataCollector` | 数据采集器接口契约 |
| `BaseDataCollector` | 采集器抽象基类，提供通用生命周期管理 |
...
DataCollectionService.collectCurrentContextData()
  ...
  │     └─ FloatingOverlayService → MoodFaceView 表情更新（stress = LLM score / 100）
  │
...
| `calendar_past_days` | 1 | 日历查询过去天数 |
| `calendar_future_days` | 1 | 日历查询未来天数 |
...
应用在 `assets/` 目录下包含两个 LLM 提示词模板文件：

| 文件 | 用途 |
|---|---|
| `prompt.txt` | LLM 系统提示词，用于根据用户上下文数据生成"下一步行动"建议（Home Screen widget 推荐 + 通知文案） |
| `Preference.txt` | 用户偏好描述模板，作为 LLM 上下文注入，帮助生成更贴合用户习惯的建议 |
